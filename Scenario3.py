...
def get_reward(cell_type, is_terminal, packages_remaining):
    if is_terminal:
        if packages_remaining == 0:
            ...
            return -100  # Terminated early due to wrong order
    elif cell_type > 0:
        return 25  # Collected correct next package
...
def main():
    ...
    log_file_path = os.path.join("logs", "scenario3_log.txt")
    with open(log_file_path, "w") as log_file:
        ...
        for episode in range(EPISODES):
            ...
            while not fourRoomsObj.isTerminal():
                ...
                packages_remaining = next_state[2]
                reward = get_reward(cell_type, is_terminal, packages_remaining)
    ...
    fourRoomsObj.showPath(-1, savefig="./plots/scenario3_path.png")

    # Plot rewards
    plt.plot(episode_rewards)
    plt.title("Scenario 3: Episode Rewards")
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.savefig("./plots/scenario3_rewards.png")
    plt.close()
