...
def train_strategy(strategy, alpha, gamma, epsilon_decay, log_file):
    ...
    env = FourRooms('rgb', stochastic='-stochastic' in sys.argv)
    ...
    for episode in range(EPISODES):
        ...
        while not env.isTerminal():
            ...
            reward = get_reward(cell_type, is_terminal, next_state[2])
...
def main():
    ...
    with open(log_file_path, "w") as log_file:
        print("Scenario 3: ε-greedy vs Softmax Exploration Comparison")
        ...
        plt.title("Scenario 3: Best Run Only")
        plt.xlabel("Episode")
        ...
        # Show or save best path
        if '-show' in sys.argv:
            best_env.showPath(-1)
        else:
            best_env.showPath(-1, savefig="./best_plots/scenario_3_best_run_path.png")
